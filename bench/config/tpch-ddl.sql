/*
TPC-H DDL: tables + indexes created together.

Unlike the default BenchBase strategy (create tables, load data, then create
indexes via afterload), this DDL creates indexes upfront.  LineairDB
automatically populates secondary indexes on every INSERT, so the data is
indexed during the load phase without a separate post-load reindex step.
*/
...
-- Indexes (originally from postload-mysql.sql, moved here to be created before loading)
