...
public:
  ...
  inline bool is_aborted() const { 
    return tx != nullptr && tx->IsAborted();
  }
...
private:
  ...
  std::string build_prefixed_key(const std::string& key) const;
  std::optional<std::string> strip_table_prefix(std::string_view key) const;
};
