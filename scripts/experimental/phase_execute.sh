...
# Phase: execute (BenchBase --execute)
# Usage: scripts/experimental/phase_execute.sh [mysqld_port]
# Env:
#  GEN_CONFIGS=true|false   Generate configs if missing (default true)
#  PRESERVE_WORKDIRS=true|false  Keep workdir (default false)
...
PHASE_LABEL=${PHASE_LABEL:-execute}
SUFFIX=""
...
(
  ...
  if [ -n "$latest_summary" ]; then
    ...
    run_prefix=${sum_base%.summary.json}
